...
export default function EditContentPage({ 
  ...
  params: Promise<{ id: string; subfolderId: string; contentId: string }> 
}) {
  const router = useRouter();
  const { id: folderId, subfolderId, contentId } = use(params);
  ...
  useEffect(() => {
    loadData();
  }, [folderId, subfolderId, contentId]);
  ...
  const loadData = async () => {
    try {
      ...
      // Carregar conteúdo
      const contentData = await studyContentService.getById(contentId);
      if (!contentData) {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      await studyContentService.update(contentId, {
        title: content.title,
  ...
  if (loading || !folder || !subfolder || !content) {
    return (
      <ProtectedRoute>
        <DashboardLayout>
      ...
      <DashboardLayout>
        <div className="max-w-3xl mx-auto">
          ...
          <nav className="flex items-center gap-2 text-sm text-gray-500 mb-6">
            ...
            <span>/</span>
            <span className="text-gray-900">Editar Conteúdo</span>
          </nav>
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 text-gray-900"
                  placeholder={getContentTypeInfo(content.type).placeholder}
                  required
                />
